...
class LlamaForCausalLMWithPretrain(LlamaForCausalLM):
    ...
    def __init__(self, config: LlamaConfig = None):
        ckpt_path = config.checkpoint_name_or_path
        config.checkpoint_name_or_path = None
        super().__init__(config)
        
        if os.path.exists(ckpt_path):
            param = load_checkpoint(ckpt_path, filter_prefix = ["lm_head", "model.tok_embeddings"])

            try:
                load_param_into_net(self, param)
                logger.info("weights in %s are loaded", ckpt_path)
            except RuntimeError:
                logger.error("the given config and weights in %s are"
                            " mismatched, and weights load failed", ckpt_path)
        elif ckpt_path not in self._support_list:
            raise ValueError(f"{ckpt_path} is not a supported default model"
